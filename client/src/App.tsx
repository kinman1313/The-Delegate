...
import { callLLMApi, getConversations, createConversation, getConversations as getConversation, addMessage } from './services/apiService';
...
function App() {
    ...
    const loadConversations = async (authToken: string) => {
        try {
            ...
            if (conversationsData.length > 0 && !currentConversationId) {
                ...
                // Explicitly indicate both parameters are intended
                await loadConversation(conversationsData[0]._id, authToken);
            }
        } catch (error) {
    ...
    const loadConversation = async (conversationId: string, authToken: string | null): Promise<void> => {
        try {
            const { conversation, files } = await getConversation(conversationId, authToken || token as string);
