...
const MultiModalInterface: React.FC<MultiModalInterfaceProps> = ({ conversationId, onSendMessage }) => {
    const [uploadedFiles, setUploadedFiles] = useState<FileItem[]>([]);
    const [contextItems, setContextItems] = useState<ContextItem[]>([]);
    const [selectedItem, setSelectedItem] = useState<FileItem | null>(null);
    const [isSelecting, setIsSelecting] = useState<boolean>(false);
    const [selectionType, setSelectionType] = useState<string | null>(null);
    const [selectionStart, setSelectionStart] = useState<Coordinates | Range | null>(null);
    ...
    const loadContextItems = async (): Promise<void> => {
        try {
    ...
    const handleStartSelection = (type: string, e?: React.MouseEvent): void => {
        if (!selectedItem || !documentViewerRef.current) return;
    ...
    const handleEndSelection = async (e: React.MouseEvent): Promise<void> => {
        if (!isSelecting || !selectedItem || !documentViewerRef.current) return;

        if (selectionType === 'image' && selectionStart && 'x' in selectionStart) {
            const { left, top } = documentViewerRef.current.getBoundingClientRect();
            const end = {
                x: e.clientX - left,
                y: e.clientY - top
    ...
    const handleGenerateVisualization = async (dataItem: FileItem): Promise<void> => {
        if (!dataItem) return;
    ...
    const renderItemThumbnail = (item: FileItem): JSX.Element => {
        switch (item.type) {
            case 'image':
                return (
    ...
    const renderDocumentViewer = (): JSX.Element => {
        if (!selectedItem) {
            return (
            ...
            case 'image':
                return (
                    <div
                        ...
                        onMouseMove={(e) => {
                            if (isSelecting && selectionType === 'image' && documentViewerRef.current) {
                                const { left, top } = documentViewerRef.current.getBoundingClientRect();
                                setSelectionEnd({
                                    x: e.clientX - left,
                                    y: e.clientY - top
                    ...
                    >
                        ...
                        {isSelecting && selectionType === 'image' &&
                            selectionStart && 'x' in selectionStart &&
                            selectionEnd && (
                                <div
    ...
    const renderVisualizations = (): JSX.Element | null => {
